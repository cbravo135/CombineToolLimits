...
void DrawUpperLimits(TString LimitType) 
{
    ...
    for(Int_t obji=3; obji<11 ;obji++){
        ...
        else if(LimitType=="FullCLs"){inputfile="OutputLogFiles/FullCLs_Mul"+nobj+"_CL95.log";}
        //else{ErrorMessage();}
        ...
        gr2s->GetYaxis()->SetRangeUser(0.01,800);
        gr2s->SetLineStyle(2);
        gr2s->SetLineWidth(2);
        ...
        float tsize=0.035;
        TLatex *tex = new TLatex(t1,t2,"35.9 fb^{-1} (13 TeV)");
        tex->SetNDC();
        tex->SetTextFont(42);
